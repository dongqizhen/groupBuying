<template>
    <ul>
        ...
        <li @click="jumpMainParams">
            <a>
                <span>重要规格：</span>
                <cube-input placeholder="请选择或输入重要规格" :disabled="true" v-model="info.mainParamsName" class="needsclick">
                    <i slot="append"></i>
                </cube-input>
            </a>
        </li>
        <li class="clinic">
            <group>
                <x-textarea title="临床应用:" placeholder="为保证医院的采购质量与效率，请详细填写耗材的临床用途" v-model="info.application" @on-change="saveApplicationValue" autosize class="needsclick"></x-textarea>
            </group>
        </li>
    </ul>
</template>
...
export default {
  ...
  computed: {
    info() {
      return this.$store.state.page.uploadProduct.HCTG;
    }
  },
  watch: {
    info() {
      console.log("变化数据：", this.$store.state.page.uploadProduct.HCTG);
    }
  },
  components: {
    Group,
  ...
  methods: {
    ...mapMutations([
      "setTransition",
      "uploadProductHCTGPriceSave",
      "uploadProductHCTGPriceIsOpenSave",
      "uploadProductHCTGApplicationSave"
    ]),
    savePriceValue() {
      this.uploadProductHCTGPriceSave(this.info.price);
    },
    saveApplicationValue() {
      this.uploadProductHCTGApplicationSave(this.info.application);
    },
    saveIsOpenValue() {
      this.uploadProductHCTGPriceIsOpenSave(!this.info.isOpen);
    },
    jumpProductCateGory() {
      if (this.$route.query.groupTypeCode) {
        ...
        this.$router.push({
          ...
          query: {
            ...
            page: "uploadProduct",
            vuexSelectValue: this.$store.state.page.uploadProduct.HCTG
              .productLine
          }
        });
      } else {
      ...
      if (this.info.productLineId) {
        ...
        this.$router.push({
          ...
          query: {
            ...
            groupTypeCode: this.$route.query.groupTypeCode,
            page: "uploadProduct",
            vuexSelectValue: this.$store.state.page.uploadProduct.HCTG.brand
          }
        });
      } else {
        Toast("请先选择分类");
      ...
      if (this.info.brandId) {
        ...
        this.$router.push({
          ...
          query: {
            ...
            groupTypeCode: this.$route.query.groupTypeCode,
            page: "uploadProduct",
            vuexSelectValue: this.$store.state.page.uploadProduct.HCTG.model
          }
        });
      } else {
        Toast("请先选择品牌");
      ...
      if (this.$route.query.groupTypeCode) {
        ...
        this.$router.push({
          ...
          query: {
            groupTypeCode: this.$route.query.groupTypeCode,
            page: "uploadProduct",
            vuexSelectValue: this.$store.state.page.uploadProduct.HCTG.params
          }
        });
      } else {
        Toast("请先选择团购产品类型");
  ...
  activated() {
    this.info.productLineName = _.join(
      _.map(
        this.$store.state.page.uploadProduct.HCTG.productLine,
        this.$store.state.page.uploadProduct.HCTG.productLine[0].aliasName
          ? "aliasName"
          : "productLineName"
      ),
      ","
    ...
    this.info.brandName = _.join(
      _.map(
        this.$store.state.page.uploadProduct.HCTG.brand,
        this.$store.state.page.uploadProduct.HCTG.brand[0].aliasName
          ? "aliasName"
          : "brandName"
      ),
      ","
    ...
    this.info.brandId = _.join(
      _.map(this.$store.state.page.uploadProduct.HCTG.brand, "brandId"),
      ","
    );
    this.info.modelName = _.join(
      _.map(this.$store.state.page.uploadProduct.HCTG.model, "name"),
      ","
    );
    this.info.mainParamsName = _.join(
      _.map(this.$store.state.page.uploadProduct.HCTG.params, "name"),
      ","
    );
     this.$nextTick().then(() => {
      this.$parent.refresh();
