    ...
    <ul>
        ...
        <li @click="jumpSelectBrand">
            <a>
                ...
                <cube-input placeholder="请选择服务商" :disabled="true" v-model="info.brandName" class="needsclick">
                    <i slot="append"></i>
                </cube-input>
            </a>
        </li>
        ...
        <li class="clinic">
            <group>
                <x-textarea title="应用方向:" placeholder="为保证医院的采购质量与效率，请详细填写咨询应用方向" v-model="info.application" @on-change="saveApplicationValue" autosize class="needsclick"></x-textarea>
            </group>
        </li>
    </ul>
</template>
    ...
    export default {
        ...
        watch: {
            info() {
                console.log(
                    "变化数据：",
                    this.$store.state.page.uploadProduct.ZXTG
                );
            }
        },
        components: {
        ...
        activated() {
            this.info.productLineName = _.join(
                _.map(
                    ...
                    this.$store.state.page.uploadProduct.ZXTG.productLine[0]
                        .aliasName
                        ? "aliasName"
                        : "productLineName"
                ),
