<template>
    <ul>
        ...
        <li class="clinic">
            <group>
                <x-textarea title="应用需求:" placeholder="为保证医院的采购质量与效率，请详细填写信息化的临床用途" v-model="info.application" @on-change="saveApplicationValue" autosize class="needsclick"></x-textarea>
            </group>
        </li>
    </ul>
</template>
...
export default {
  ...
  computed: {
    info() {
      return this.$store.state.page.uploadProduct.XXHTG;
    }
  },
  watch: {
    info() {
      console.log("变化数据：", this.$store.state.page.uploadProduct.XXHTG);
    }
  },
  components: {
    Group,
  ...
  methods: {
    ...mapMutations([
      "setTransition",
      "uploadProductXXHTGPriceSave",
      "uploadProductXXHTGPriceIsOpenSave",
      "uploadProductXXHTGApplicationSave"
    ]),
    savePriceValue() {
      this.uploadProductXXHTGPriceSave(this.info.price);
    },
    saveApplicationValue() {
      this.uploadProductXXHTGApplicationSave(this.info.application);
    },
    saveIsOpenValue() {
      this.uploadProductXXHTGPriceIsOpenSave(!this.info.isOpen);
    },
    jumpProductCateGory() {
      if (this.$route.query.groupTypeCode) {
        ...
        this.$router.push({
          ...
          query: {
            ...
            page: "uploadProduct",
            vuexSelectValue: this.$store.state.page.uploadProduct.XXHTG
              .productLine
          }
        });
      } else {
      ...
      if (this.info.productLineId) {
        ...
        this.$router.push({
          ...
          query: {
            ...
            groupTypeCode: this.$route.query.groupTypeCode,
            page: "uploadProduct",
            vuexSelectValue: this.$store.state.page.uploadProduct.XXHTG.brand
          }
        });
      } else {
        Toast("请先选择分类");
      ...
      if (this.info.brandId) {
        ...
        this.$router.push({
          ...
          query: {
            ...
            groupTypeCode: this.$route.query.groupTypeCode,
            page: "uploadProduct",
            vuexSelectValue: this.$store.state.page.uploadProduct.XXHTG.model
          }
        });
      } else {
        Toast("请先选择品牌");
      ...
      if (this.$route.query.groupTypeCode) {
        ...
        this.$router.push({
          ...
          query: {
            groupTypeCode: this.$route.query.groupTypeCode,
            page: "uploadProduct",
            vuexSelectValue: this.$store.state.page.uploadProduct.XXHTG.params
          }
        });
      } else {
        Toast("请先选择团购产品类型");
  ...
  activated() {
    this.info.productLineName = _.join(
      _.map(
        this.$store.state.page.uploadProduct.XXHTG.productLine,
        this.$store.state.page.uploadProduct.XXHTG.productLine[0].aliasName
          ? "aliasName"
          : "productLineName"
      ),
      ","
    ...
    this.info.brandName = _.join(
      _.map(
        this.$store.state.page.uploadProduct.XXHTG.brand,
        this.$store.state.page.uploadProduct.XXHTG.brand[0].aliasName
          ? "aliasName"
          : "brandName"
      ),
      ","
    ...
    this.info.brandId = _.join(
      _.map(this.$store.state.page.uploadProduct.XXHTG.brand, "brandId"),
      ","
    );
    this.info.modelName = _.join(
      _.map(this.$store.state.page.uploadProduct.XXHTG.model, "name"),
      ","
    );
    this.info.mainParamsName = _.join(
      _.map(this.$store.state.page.uploadProduct.XXHTG.params, "name"),
      ","
    );
    this.$nextTick().then(() => {
      this.$parent.refresh();
