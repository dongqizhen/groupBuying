      ...
      <scroller :class="this.$route.query.come==1||this.$store.state.userType=='company'?'height':''">
        ...
        <div class="proBaseInfo">
          ...
          <div class="proPrice">
            ...
            <span :class="data.isOpen || $USER_INFO.userid==data.companyUserId?'price':'textStyle'">{{data.isOpen || $USER_INFO.userid==data.companyUserId ?(data.price?data.price.split(".")[0]:""):"团购办现场宣布价格"}}</span>
            <span :class="data.isOpen ||$USER_INFO.userid==data.companyUserId?'point':'textStyle'">{{data.isOpen || $USER_INFO.userid==data.companyUserId?(data.price?'.'+data.price.split(".")[1]:""):""}}</span>
        ...
        case "加入我的团购需求表":
          _getData(
            "/server_pro/groupPurchaseHospital!request.action",
            {
              ...
              params: { groupPurchaseCompanyProductId: this.data.id }
            },
            data => {
              console.log(data);
              this.setTransition("turn-on");
